...
object ShellExecutor {
    ...
    val isTermuxRuntimeAvailable: Boolean
        get() = TermuxRuntimeManager.isRuntimeInstalled
    ...
    fun execute(command: String, timeoutSeconds: Long = 120L, workDir: File? = null): CommandResult {
        ...
        try {
            ...
            android.util.Log.e("VoltShell", "execute('$command') a échoué", e)
            return CommandResult(
                command = command,
                output = "",
                error = e.message,
    ...
    fun executeStreamingFull(
        ...
        try {
            ...
            android.util.Log.e("VoltShell", "streaming('$command') a échoué", e)
            return CommandResult(
                command = command,
                output = "",
                error = e.message,
    ...
    private fun buildProcess(command: String, workDir: File?): Pair<String, ProcessBuilder> {
        return when {
            ...
            // Si proot est indisponible on échoue proprement : exécuter les
            // binaires du bootstrap en direct est impossible (interpréteur ELF
            // pointant vers le chemin virtuel inexistant sur l'hôte).
            TermuxRuntimeManager.isRuntimeInstalled -> {
                val pb = TermuxRuntimeManager.buildProcessBuilder(command, workDir)
                if (pb != null) Pair(command, pb)
                else throw IllegalStateException(
                    "Runtime embarqué présent mais proot indisponible (${TermuxRuntimeManager.PROOT_PATH}). Réessayez après réinitialisation."
                )
            }
